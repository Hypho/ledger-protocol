...
Ledger separates framework version checks from repository publishing.
...
Default for every Ledger project.
...
```bash
bash .ledger/bin/ledger-check.sh --project

# In the Ledger framework repository:
...
This layer is intentionally documented, not built into the default Ledger checks.
...
Ledger uses a Keep a Changelog style for `CHANGELOG.md`.
...
```bash
git tag -a vX.Y.Z -m "Ledger vX.Y.Z"
```
...
For the Ledger public repository:
