#!/bin/bash
# Controlled Ledger state operations for the v1.x Markdown state source.
...
usage() {
  cat <<'EOF'
Usage: bash .ledger/bin/ledger-state.sh <command> [args]
...
validate_artifacts() {
  ...
  case "$PHASE" in
    pid)
      [ -f "$LEDGER_ROOT/.ledger/specs/${FEATURE}-pid.md" ] || fail "state 声明 pid，但 specs/${FEATURE}-pid.md 不存在"
      ;;
    contract|build|build-complete)
      [ -f "$LEDGER_ROOT/.ledger/contracts/${FEATURE}.md" ] || fail "state 声明 ${PHASE}，但 contracts/${FEATURE}.md 不存在"
      bash "$ROOT/.ledger/bin/ledger-lint-contract.sh" "$LEDGER_ROOT/.ledger/contracts/${FEATURE}.md" >/dev/null
      ;;
    verify-pass)
      local verify="$LEDGER_ROOT/.ledger/knowledge/${FEATURE}-verify.md"
      [ -f "$verify" ] || fail "state 声明 verify-pass，但 knowledge/${FEATURE}-verify.md 不存在"
      bash "$ROOT/.ledger/bin/ledger-lint-verify.sh" "$verify" >/dev/null
      grep -Eq '^verdict = PASS$|MANUAL OVERRIDE' "$verify" || fail "verify-pass 需要 verdict = PASS 或 MANUAL OVERRIDE"
      ;;
    shipped)
      [ ! -f "$LEDGER_ROOT/.ledger/contracts/${FEATURE}.md" ] || fail "shipped 阶段不应保留 active contract：contracts/${FEATURE}.md"
      ;;
  esac
}
...
validate_state() {
  ...
  validate_artifacts
  pass "Ledger state validate passed"
}
...
write_state_atomic() {
  ...
  echo "Ledger state diff:"
  diff -u "$STATE_FILE" "$tmp" || true

  LEDGER_STATE_FILE="$tmp" LEDGER_ROOT="$LEDGER_ROOT" bash "$ROOT/.ledger/bin/ledger-state.sh" validate >/dev/null
  mv "$tmp" "$STATE_FILE"
}
...
cmd_complete() {
  ...
  contract_ref=".ledger/contracts/archive/${FEATURE}.md"
  if [ -f "$LEDGER_ROOT/.ledger/contracts/${FEATURE}.md" ]; then
    contract_ref=".ledger/contracts/${FEATURE}.md"
    echo "⚠️ active contract still exists and should be archived: $contract_ref"
  fi

  {
    echo "| $FEATURE | $STARTED_AT | $(date '+%Y-%m-%d %H:%M') | $contract_ref | see verify | .ledger/knowledge/${FEATURE}-verify.md |"
...
cmd_fail_verify() {
  ...
  errors_dir="$LEDGER_ROOT/.ledger/knowledge/errors"
  mkdir -p "$errors_dir"
...

  {
    ...
    echo "- verify: .ledger/knowledge/${FEATURE}-verify.md"
    echo "- verdict: FAIL"
  ...
  write_state_atomic "$FEATURE" "build" "$STARTED_AT" "修复 verify FAIL：$summary" "verify FAIL，见 .ledger/knowledge/errors/$(basename "$error_file")" "$queue_file" "$completed_file"
  rm -rf "$tmpdir"
...
run_fixtures() {
  ...
  expect_success "validate idle" env LEDGER_ROOT="$tmp/idle" bash "$ROOT/.ledger/bin/ledger-state.sh" validate
  expect_success "enqueue" env LEDGER_ROOT="$tmp/idle" bash "$ROOT/.ledger/bin/ledger-state.sh" enqueue "fixture-login"
  expect_failure "duplicate enqueue" env LEDGER_ROOT="$tmp/idle" bash "$ROOT/.ledger/bin/ledger-state.sh" enqueue "fixture-login"

  mkdir -p "$tmp/phase/.ledger/specs"
  write_fixture_state "$tmp/phase" "[名称]" "待开始"
  env LEDGER_ROOT="$tmp/phase" bash "$ROOT/.ledger/bin/ledger-state.sh" enqueue "fixture-login" >/dev/null
  echo "# PID" > "$tmp/phase/.ledger/specs/fixture-login-pid.md"
  expect_success "set phase pid" env LEDGER_ROOT="$tmp/phase" bash "$ROOT/.ledger/bin/ledger-state.sh" set-phase pid
  expect_failure "illegal phase jump" env LEDGER_ROOT="$tmp/phase" bash "$ROOT/.ledger/bin/ledger-state.sh" set-phase build-complete

  mkdir -p "$tmp/fail/.ledger/contracts" "$tmp/fail/.ledger/knowledge"
  write_fixture_state "$tmp/fail" "fixture-login" "build-complete"
  cp "$ROOT/.ledger/tests/fixtures/contract/valid-contract.md" "$tmp/fail/.ledger/contracts/fixture-login.md"
  echo "verdict = FAIL" > "$tmp/fail/.ledger/knowledge/fixture-login-verify.md"
  expect_success "fail verify" env LEDGER_ROOT="$tmp/fail" bash "$ROOT/.ledger/bin/ledger-state.sh" fail-verify

  mkdir -p "$tmp/complete/.ledger/knowledge"
  write_fixture_state "$tmp/complete" "fixture-login" "verify-pass"
  cp "$ROOT/.ledger/tests/fixtures/verify/valid-pass.md" "$tmp/complete/.ledger/knowledge/fixture-login-verify.md"
  expect_success "complete" env LEDGER_ROOT="$tmp/complete" bash "$ROOT/.ledger/bin/ledger-state.sh" complete
...
case "$cmd" in
  ...
  -h|--help|help|"")
    ...
    ;;
  *)
    echo "Unknown Ledger state command: $cmd" >&2
    usage
    exit 2
    ;;
esac
